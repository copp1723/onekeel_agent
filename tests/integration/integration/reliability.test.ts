...
import { retry } from '../../../src/src/../src/../utils/retry.js';
import { CircuitBreaker, CircuitState } from '../../../src/src/../src/../utils/circuitBreaker.js';
...
describe('Retry and Circuit Breaker Integration', () => {
  ...
  it('should open circuit after persistent failures despite retries', async () => {
    ...
    await expect(circuitBreaker.execute(anotherFn)).rejects.toThrow(
      'Circuit test-integration-failure is OPEN'
    );
  ...
  it('should handle multiple services with separate circuit breakers', async () => {
    ...
    await expect(serviceACircuit.execute(serviceA)).rejects.toThrow('Circuit service-a is OPEN');
