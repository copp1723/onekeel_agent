...
INSIGHT_RESPONSE_SCHEMA = {
    ...
    "properties": {
        "summary": {"type": "string", "description": "A clear, concise summary of the main insight"},
        "chart_data": {
            "type": "object",
        ...
        "recommendation": {"type": "string", "description": "A specific, actionable recommendation based on the data"},
        "risk_flag": {"type": "boolean", "description": "Flag indicating whether the insight reveals a potential risk"},
        "confidence_score": {
            "type": "number",
            "minimum": 0,
            "maximum": 1,
...
def build_prompt(
    analytics_summary: Union[str, Dict[str, Any]], query: str, custom_params: Optional[Dict[str, Any]] = None
) -> str:
    """
    ...
    return INSIGHT_PROMPT_TEMPLATE.format(
        analytics_summary=analytics_summary, query=query, custom_instructions=custom_instructions
    )
...
def validate_insight_response(response: Dict[str, Any]) -> None:
    ...
    try:
        ...
        raise jsonschema.exceptions.ValidationError(f"Invalid insight response format: {str(e)}") from e
