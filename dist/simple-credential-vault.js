 ...
 */
export function initializeEncryption(key) {
    if (key) {
...
export async function getCredentialById(id, userId) {
    ...
    const [credential] = await db.select()
        .from(credentials)
        .where(and(eq(credentials.id, id), eq(credentials.userId, userId), eq(credentials.active, true)));
    if (!credential) {
 ...
 */
export async function getCredentials(userId, platformFilter) {
...
export async function updateCredential(id, userId, data, options) {
    ...
    if (options?.label !== undefined)
        updateData.label = options.label;
...
//# sourceMappingURL=simple-credential-vault.js.map