...
export async function getApiKey(keyName) {
    try {
        ...
        const result = await db.execute(sql `SELECT key_value FROM api_keys WHERE key_name = ${keyName} LIMIT 1`);
        if (result.length > 0) {
 ...
 */
export async function getDealerCredentials(dealerId) {
    try {
 ...
 */
export async function saveDealerCredentials(dealerId, credentials) {
    try {
