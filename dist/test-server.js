...
app.post('/submit-task', function (req, res) {
    try {
        ...
        // Generate task ID
        var taskId = crypto_1.default.randomUUID();
        // Log task in memory
        var taskType = task.toLowerCase().includes('crawl') ? 'web_crawling' :
...
app.listen(PORT, function () {
    console.log("AgentFlow Test API server running on port ".concat(PORT));
