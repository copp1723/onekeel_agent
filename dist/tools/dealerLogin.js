 ...
 */
function identifyDealerSystem(dealerId) {
    const lowercaseId = dealerId.toLowerCase();
    // Check if the dealer ID contains a known system name
    const matchedSystem = Object.keys(DEALER_SYSTEMS).find(system => lowercaseId.includes(system.toLowerCase()));
    if (matchedSystem) {
        return {
            key: matchedSystem,
