...
router.post('/retry/:emailLogId', async (req, res) => {
    try {
        ...
        // Send the email
        const result = await sendWorkflowCompletionEmail(workflow.id, emailLog.recipientEmail);
        if (!result.success) {
...
router.post('/test', async (req, res) => {
    try {
        ...
        // Send the email
        const result = await sendWorkflowCompletionEmail(workflow.id, recipientEmail);
        res.json({
...
//# sourceMappingURL=fixed-emails.js.map