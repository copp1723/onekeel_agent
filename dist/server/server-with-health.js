...
// Middleware
app.use(cors({
  origin: 'http://localhost:5000', // Allow the frontend to connect
...
// API endpoint for health dashboard frontend
app.get('/api/health-monitoring/summary', async (req, res) => {
  try {
...
app.get('/api/health-monitoring/checks', async (req, res) => {
  try {
...
app.post('/api/health-monitoring/checks/run', async (req, res) => {
  try {
...
app.get('/api/health-monitoring/logs/:checkId', async (req, res) => {
  try {
...
async function storeHealthCheckResult(result) {
  if (!result) {
    console.error('Health check result is undefined or null');
    return;
  }
  ...
  try {
    ...
    if (existingChecks && existingChecks.rows && existingChecks.rows.length > 0) {
      ...
      await db.execute(sql`
        ...
        SET
          ...
          last_checked = ${result.lastChecked.toISOString()},
...
async function setupTables() {
  try {
    // Set up health_checks table
    const healthChecksExists = await checkTableExists('health_checks');
    if (!healthChecksExists) {
      await createHealthChecksTable();
    }
    
    // Set up health_logs table
    const healthLogsExists = await checkTableExists('health_logs');
    if (!healthLogsExists) {
      await createHealthLogsTable();
    }
