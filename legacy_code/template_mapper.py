...
class TemplateMapper:
    ...
    def __init__(self, dictionary_path: Optional[str] = None):
        ...
        Args:
            dictionary_path: Path to the data dictionary JSON file. If None, will try multiple locations.
        """
        # Try to find the data dictionary in various locations
        if dictionary_path is None:
            # Try to import from config
            try:
                from legacy_code.config import DATA_DICTIONARY_PATH
                dictionary_path = DATA_DICTIONARY_PATH
            except (ImportError, AttributeError):
                # If that fails, try common locations
                possible_paths = [
                    "assets/data_dictionary.json",
                    "legacy_code/data_dictionary.json",
                    Path(__file__).parent / "data_dictionary.json",
                    Path.cwd() / "assets" / "data_dictionary.json",
                    Path.cwd() / "legacy_code" / "data_dictionary.json"
                ]
        ...
        if dictionary_path and os.path.exists(str(dictionary_path)):
            with open(str(dictionary_path), 'r') as f:
                self.dictionary = json.load(f)
        else:
            raise FileNotFoundError(f"Dictionary file not found. Tried path: {dictionary_path}")
