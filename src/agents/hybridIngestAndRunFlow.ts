...
import { CRMPlatform, EnvVars } from '../types.js.js';
...
export async function emailIngestAndRunFlow(
  ...
  try {
    ...
    return await emailIngestionCircuitBreaker.execute(async () => {
      ...
      const errorMessage = isError(error) ? (error instanceof Error ? isError(error) ? (error instanceof Error ? error.message : String(error)) : String(error) : String(error)) : String(error);
          if (error instanceof ReportNotFoundError) {
            logger.error(`No report found in email: ${isError(error) ? (error instanceof Error ? isError(error) ? (error instanceof Error ? error.message : String(error)) : String(error) : String(error)) : String(error)}`);
          ...
          } else if (error instanceof Error && (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)).includes('Missing required email configuration')) {
            logger.error(`Email configuration error: ${(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))}`);
          ...
          } else if (error instanceof Error && (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)).includes('Authentication failed')) {
            logger.error(`Email authentication error: ${(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))}`);
            throw new Error(`Email ingestion failed: Authentication failed. Please check your email credentials.`);
          } else if (error instanceof Error && (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)).includes('connect')) {
            logger.error(`Email connection error: ${(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))}`);
          ...
          } else if (error instanceof Error && (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)).includes('circuit is open')) {
            logger.warn(`Email service circuit is open: ${(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))}`);
            ...
            throw new Error(`Email ingestion failed: ${error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)}`);
          }
        }
      }, {
        retries: 2,
    ...
    if (error.name === 'CircuitOpenError') {
      throw new Error(`Email ingestion service is currently unavailable. The circuit is open due to multiple failures.`);
    }
    // Propagate the error with additional context
    if (error instanceof Error) {
      throw new Error(`Email ingestion failed: ${(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))}`);
    } else {
