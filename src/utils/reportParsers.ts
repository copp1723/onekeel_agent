...
import { retry } from './retry.js.js';
import { CircuitBreaker } from './circuitBreaker.js.js';
      ...
      try {
        ...
        worksheet.eachRow((row, rowNumber) => {
          ...
          if (rowNumber === 1 && options.headers !== false) {
            ...
            row.eachCell((cell, colNumber) => {
              const header = options.headers !== false 
                ? headers[colNumber - 1] 
                : `Column${colNumber}`;
        ...
        logger.info(`Parsed ${records.length} records from Excel file: ${path.basename(filePath)}`);
        return records;
      } catch (error) {
  ...
  return parserCircuitBreaker.execute(async () => {
    return retry(async () => {
      try {
        ...
        logger.info(`Extracted ${text.length} characters from PDF file: ${path.basename(filePath)}`);
...
function extractTabularDataFromPDF(
  ...
  try {
    ...
    // Extract headers
    const headers = options.columns || (headerLine >= 0 
      ? lines[headerLine].split(/\s{2,}/).map(h => h.trim())
      : []);
    // Process data rows
    const records: Record<string, any>[] = [];
