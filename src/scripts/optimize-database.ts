...
import { 
  ...
  optimizeTable, 
  getDatabaseStats 
} from '../services/dbOptimizationService.js.js';
/**
...
async function optimizeDatabase() {
  ...
  try {
    ...
    await createIndexIfNotExists('task_logs', 'idx_task_logs_status_created_at', ['status', 'created_at']);
    await createIndexIfNotExists('task_logs', 'idx_task_logs_task_type', ['task_type']);
    // Workflows indexes
    await createIndexIfNotExists('workflows', 'idx_workflows_status_created_at', ['status', 'created_at']);
    await createIndexIfNotExists('workflows', 'idx_workflows_user_id_status', ['user_id', 'status']);
    // Email queue indexes
    await createIndexIfNotExists('email_queue', 'idx_email_queue_status_created_at', ['status', 'created_at']);
    ...
    await createIndexIfNotExists('email_logs', 'idx_email_logs_recipient_email', ['recipient_email']);
    // Health checks indexes
    await createIndexIfNotExists('health_checks', 'idx_health_checks_last_checked', ['last_checked']);
    // Health logs indexes
    await createIndexIfNotExists('health_logs', 'idx_health_logs_check_id_timestamp', ['check_id', 'timestamp']);
    // Dealer credentials indexes
    await createIndexIfNotExists('dealer_credentials', 'idx_dealer_credentials_dealer_id_platform', ['dealer_id', 'platform']);
