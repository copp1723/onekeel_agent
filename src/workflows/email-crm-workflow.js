/**
 * Email-Only CRM Workflow Configuration
 * 
 * This file defines a workflow configuration that uses the email-only ingestion approach
 * to retrieve CRM reports, analyze them, and generate insights.
 */

import fs from 'fs';
import path from 'path';
import emailIngestForCRM from '../agents/emailIngestForCRM.js';

/**
 * Configuration for the Email-Only CRM Workflow
 * 
 * This workflow consists of three steps:
 * 1. Fetch CRM Report - Retrieve the CRM report using email-only ingestion
 * 2. Process Data - Parse and process the CSV data
 * 3. Generate Insights - Analyze the data using the automotive analyst
 */
export const emailCRMWorkflow = {
  name: 'Email-Only CRM Analysis',
  description: 'Fetch CRM reports using email ingestion, then analyze for insights',
  steps: [
    {
      id: 'fetch-crm-report',
      name: 'Fetch CRM Report',
      description: 'Retrieve CRM reports using email ingestion',
      handler: async (context) => {
        const platform = context.platform || 'VinSolutions';
        console.log(`Executing email-only ingestion for ${platform}...`);
        
        try {
          // Use the email-only ingestion agent to fetch the report
          const reportPath = await emailIngestForCRM(platform);
          ...
          console.error(`Error during email ingestion: ${error.message}`);
          return {
            success: false,
            error: error.message,
            timestamp: new Date().toISOString()
...
export default emailCRMWorkflow;
