...
// Define strict types for schedule metadata
 ...
 */
export async function retrySchedule(scheduleId: string): Promise<Schedule | undefined> {
  try {
    // Get the schedule
    const [schedule] = await db
      .select()
 ...
 */
export async function getScheduleLogs(scheduleId: string): Promise<any[]> {
