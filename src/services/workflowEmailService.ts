...
import { Workflow, workflows } from '../shared/schema.js';
 ...
 */
export async function sendWorkflowSummaryEmail(options: WorkflowEmailOptions): Promise<boolean> {
  try {
 ...
 */
async function extractWorkflowData(workflow: Workflow, includeInsights: boolean): Promise<any> {
  ...
  if (workflow.context) {
    ...
    const context = workflow.context as any;
 ...
 */
export async function sendWorkflowCompletionEmail(workflowId: string, recipients: string | string[]): Promise<boolean> {
  try {
