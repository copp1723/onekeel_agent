...
const sampleVendorData: Record<string, SampleVendorData> = {
  ...
  VAUTO: {
    records: [
      { 'Report Date': '2025-05-13', 'Stock#': 'A123', 'VIN': '1HGCM82633A123456', 'Make': 'Honda', 'Model': 'Accord', 'Price': 32500, 'Cost': 29000, 'Age': 15, 'Category': 'Sedan', 'Source': 'Auction' },
...
export async function checkEmailsFromAllVendors(): Promise<EmailCheckResult> {
  try {
    ...
    if (vendors.length === 0) {
      return { success: false, message: 'No vendors configured', totalProcessed: 0, processedReports: [] };
    }
 ...
 */
async function processVendorEmails(vendor: string, vendorConfig: VendorConfig): Promise<VendorEmailResult> {
  try {
 ...
 */
async function processAttachment(attachment: EmailAttachment, extractorConfig: ExtractorConfig): Promise<AttachmentResult> {
  try {
 ...
 */
async function parseCsvFile(filePath: string, extractorConfig: ExtractorConfig): Promise<any[]> {
  try {
    // Read file content
 ...
 */
async function parseExcelFile(filePath: string, extractorConfig: ExtractorConfig): Promise<any[]> {
  try {
    // Load workbook
 ...
 */
async function parsePdfFile(filePath: string, extractorConfig: ExtractorConfig): Promise<any[]> {
  try {
    // Read file content
