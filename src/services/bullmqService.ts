...
import { jobs } from '../shared/schema.js.js';
...
export function createQueue(name: string, options: QueueOptions = {}): Queue | null {
  try {
    ...
    logger.error({ 
      ...
      errorMessage: error instanceof Error ? isError(error) ? getErrorMessage(error) : String(error) : String(error), 
      timestamp: new Date().toISOString() 
    }, `Error creating queue ${name}: ${error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)}`);
    return null;
  }
}
/**
  ...
  try {
    ...
    worker.on('failed', (job, error) => {
      logger.error({ 
        ...
        timestamp: new Date().toISOString() 
      }, `Job ${job?.id} failed in queue ${queueName}: ${(error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))}`);
      // Update job status in database if max attempts reached
      if (job && job.attemptsMade >= (job.opts.attempts || 3)) {
        updateJobStatus(job.id as string, 'failed', (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error))).catch(err => {
          logger.error({ 
            event: 'job_status_update_error', 
            jobId: job.id, 
            errorMessage: err.message, 
    ...
    logger.error({ 
      ...
      errorMessage: error instanceof Error ? isError(error) ? getErrorMessage(error) : String(error) : String(error), 
      timestamp: new Date().toISOString() 
    }, `Error creating worker for queue ${queueName}: ${error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)}`);
    return null;
  }
}
/**
...
export function createScheduler(queueName: string): QueueScheduler | null {
  try {
    ...
    logger.error({ 
      ...
      errorMessage: error instanceof Error ? isError(error) ? getErrorMessage(error) : String(error) : String(error), 
      timestamp: new Date().toISOString() 
    }, `Error creating scheduler for queue ${queueName}: ${error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)}`);
    return null;
  }
}
/**
...
async function updateJobStatus(jobId: string, status: string, errorMessage?: string): Promise<void> {
  try {
    ...
    logger.error({ 
      ...
      errorMessage: error instanceof Error ? isError(error) ? getErrorMessage(error) : String(error) : String(error), 
      timestamp: new Date().toISOString() 
    }, `Error updating job status: ${error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)}`);
  }
}
/**
 * Get a queue by name
...
export async function closeConnections(): Promise<void> {
  try {
    ...
    logger.error({ 
      ...
      errorMessage: error instanceof Error ? isError(error) ? getErrorMessage(error) : String(error) : String(error), 
      timestamp: new Date().toISOString() 
    }, `Error closing connections: ${error instanceof Error ? (error instanceof Error ? (error instanceof Error ? error.message : String(error)) : String(error)) : String(error)}`);
  }
}
