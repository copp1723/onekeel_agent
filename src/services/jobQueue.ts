...
import { jobs, taskLogs } from '../shared/schema.js.js';
...
export async function initializeJobQueue() {
  try {
    ...
    logger.info({ event: 'redis_connect_attempt', host: options.host, port: options.port, timestamp: new Date().toISOString() }, `Attempting to connect to Redis at ${options.host}:${options.port}...`);
    ...
    redisClient.on('error', (err: any) => {
      if (!inMemoryMode) {
        logger.error({ event: 'redis_connection_error', errorMessage: err.message, timestamp: new Date().toISOString() }, `Redis connection error: ${err.message}`);
      }
    });
    ...
    try {
      ...
      if (QueueScheduler) {
        ...
        logger.info({ event: 'scheduler_initialized', timestamp: new Date().toISOString() }, 'QueueScheduler initialized successfully');
      } else {
        logger.warn({ event: 'scheduler_unavailable', timestamp: new Date().toISOString() }, 'QueueScheduler not available in bullmq package');
      }
    } catch (err) {
      logger.warn({ event: 'scheduler_init_failed', errorMessage: err.message, stack: err.stack, timestamp: new Date().toISOString() }, 'QueueScheduler could not be initialized, continuing without scheduler');
    }
    inMemoryMode = false;
    ...
    logger.warn({ event: 'redis_connection_failed', errorMessage: isError(error) ? getErrorMessage(error) : String(error), stack: (error instanceof Error ? (error instanceof Error ? error.stack : undefined) : undefined), timestamp: new Date().toISOString() }, 'Redis connection failed, using in-memory job queue');
    inMemoryMode = true;
  }
...
async function setupWorker() {
  if (!inMemoryMode && redisClient) {
    try {
      ...
      logger.warn({ event: 'worker_init_failed', errorMessage: getErrorMessage(error), stack: (error instanceof Error ? (error instanceof Error ? error.stack : undefined) : undefined), timestamp: new Date().toISOString() }, 'Failed to initialize BullMQ worker');
    }
  } else {
...
async function processInMemoryJobs() {
  ...
  for (const job of pendingJobs) {
    try {
      ...
      const errorMessage = isError(error) ? (error instanceof Error ? isError(error) ? (error instanceof Error ? error.message : String(error)) : String(error) : String(error)) : String(error);
      job.attempts += 1;
      job.lastError = error instanceof Error ? isError(error) ? (error instanceof Error ? isError(error) ? (error instanceof Error ? error.message : String(error)) : String(error) : String(error)) : String(error) : String(error);
...
async function processJob(jobId: string, data: any) {
  try {
    ...
    return true;
      ...
      // Use type-safe error handling
      const errorMessage = isError(error) ? (error instanceof Error ? isError(error) ? (error instanceof Error ? error.message : String(error)) : String(error) : String(error)) : String(error);
    logger.error({ event: 'job_processing_error', jobId, errorMessage: isError(error) ? getErrorMessage(error) : String(error), stack: (error instanceof Error ? (error instanceof Error ? error.stack : undefined) : undefined), timestamp: new Date().toISOString() }, `Error processing job ${jobId}`);
...
export async function retryJob(jobId: string): Promise<boolean> {
  try {
    ...
    return true;
      ...
      // Use type-safe error handling
      const errorMessage = isError(error) ? (error instanceof Error ? isError(error) ? (error instanceof Error ? error.message : String(error)) : String(error) : String(error)) : String(error);
    logger.error({ event: 'retry_job_error', jobId, errorMessage: isError(error) ? getErrorMessage(error) : String(error), stack: (error instanceof Error ? (error instanceof Error ? error.stack : undefined) : undefined), timestamp: new Date().toISOString() }, `Error retrying job ${jobId}`);
    return false;
  }
}
